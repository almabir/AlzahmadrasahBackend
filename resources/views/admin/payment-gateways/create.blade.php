...
@section('content')
    <div class="w-full flex justify-between items-center mb-3 mt-1 pl-3">
        <div>
            <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-200">Create Payment Gateway</h3>
            <p class="text-slate-500 dark:text-slate-400">Create a new payment gateway.</p>
        </div>
    </div>
    ...
    <div class="relative flex flex-col w-full h-full overflow-scroll text-gray-700 dark:text-gray-200 bg-white dark:bg-slate-800 shadow-md rounded-lg bg-clip-border p-6">
        <form action="{{ route('admin.payment-gateways.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="api_key" id="api_key" class="mt-1 block w-full border border-slate-300 dark:border-slate-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-200 focus:outline-none" value="{{ old('api_key') }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="api_secret" id="api_secret" class="mt-1 block w-full border border-slate-300 dark:border-slate-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-200 focus:outline-none" value="{{ old('api_secret') }}">
            ...
            <div class="mb-4">
                ...
                <textarea name="config" id="config" class="mt-1 block w-full border border-slate-300 dark:border-slate-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-200 focus:outline-none">{{ old('config') ? json_encode(old('config'), JSON_PRETTY_PRINT) : '' }}</textarea>
